...
export function TextChatDetailShell({
  ...
  return (
    <div
      ...
      aria-label={title}
      onMouseDown={(e) => {
        if (e.target === e.currentTarget) router.push("/history/chats");
      }}
    >
      <div className="y-modal-sheet y-tchat-detail-sheet" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <header className="y-tchat-detail-head">
          <Link href="/history/chats" className="y-modal-back" scroll={false} aria-label="목록으로">
          ...
          <Link href="/history/chats" className="y-modal-close" scroll={false} aria-label="닫기">
            ×
          </Link>
        </header>
