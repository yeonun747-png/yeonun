/**
 * Anthropic 점사 프롬프트 캐시 — 공통 지침+캐릭터(및 고정 시스템 규칙)만 cache_control.
 * @see https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching
 *
 * Sonnet 4.6: 캐시 가능한 블록 최소 2048 토큰(미만이면 캐시 미적용·에러 없음). 일부 구형 Sonnet/Opus는 1024.
 */
...
export type AnthropicFortuneSystemBlock = {
  type: "text";
  text: string;
  cache_control: { type: "ephemeral"; ttl: "1h" };
};

export function fortuneCachedSystemBlocks(paddedCacheableText: string): AnthropicFortuneSystemBlock[] {
  return [
    {
      type: "text",
